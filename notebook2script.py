...
def notebook2script(fname=None, allFiles=None, upTo=None, target=None):
    '''Finds cells starting with `#export` and puts them into a new module
       + allFiles: convert all files in the folder
       + upTo: convert files up to specified one included
       + target: specify target directory, defaults to 'exp'
    ...
    if not allFiles: notebook2scriptSingle(fname,target)
    else:
        print('Begin...')
...
def notebook2scriptSingle(fname,target):
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(fname)
...
#################################################
# file to edit: ../{str(fname)}

'''
    for cell in code_cells: module += ''.join(cell['source'][1:]) + '\n\n'
    # remove trailing spaces
    module = re.sub(r' +$', '', module, flags=re.MULTILINE)
    if not (fname.parent/target).exists(): (fname.parent/target).mkdir()
    output_path = fname.parent/target/fname_out
